...
class EmailWorker:
    ...
    async def send_email_task(self, alert_id: int, user_email: str, symbol: str, 
        ...
        try:
            ...
            await self.alert_service.update_alert_status(alert_id, AlertStatus.SENT.value)
            logger.info(f"Email sent and status updated for {alert_id}")
            return True
            
        except Exception as e:
            await self.alert_service.update_alert_status(alert_id, AlertStatus.FAILED.value)
